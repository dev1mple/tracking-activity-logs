...
function insertAnActivityLog($pdo, $operation, $branch_id, $address, 
		$employee, $contact_number, $username) {

	$sql = "INSERT INTO activity_logs (operation, branch_id, address, 
		employee, contact_number, username) VALUES(?,?,?,?,?,?)";
...
function insertABranch($pdo, $address, $employee, $contact_number, $added_by) {
	$response = array();
	$sql = "INSERT INTO branches (address, employee, contact_number, added_by) VALUES(?,?,?,?)";
	$stmt = $pdo->prepare($sql);
	$insertBranch = $stmt->execute([$address, $employee, $contact_number, $added_by]);
	...
	if ($insertBranch) {
		...
		$insertAnActivityLog = insertAnActivityLog($pdo, "INSERT", $getBranchID['branch_id'], 
			$getBranchID['address'], $getBranchID['employee'], 
...
function updateBranch($pdo, $address, $employee, $contact_number, 
	...
	$sql = "UPDATE branches
			...
			";
	$stmt = $pdo->prepare($sql);
	$updateBranch = $stmt->execute([$address, $employee, $contact_number, 
	$last_updated, $last_updated_by, $branch_id]);
...
function deleteABranch($pdo, $branch_id) {
	...
	$insertAnActivityLog = insertAnActivityLog($pdo, "DELETE", $getBranchByID['branch_id'], 
		$getBranchByID['address'], $getBranchByID['employee'], 
